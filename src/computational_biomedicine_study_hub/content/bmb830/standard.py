"""Compact authoring contract for complete BMB830 modules."""
...
class StandardModuleSpec:
    """A dense BMB830 module with enforced academic coverage."""
    ...
    def __post_init__(self) -> None:
        minimums = {
            "objectives": (len(self.objectives), 4),
            "concepts": (len(self.concepts), 4),
            "examples": (len(self.examples), 2),
            "practices": (len(self.practices), 6),
            "mcqs": (len(self.mcqs), 8),
            "true_false": (len(self.true_false), 8),
        }
        for name, (actual, minimum) in minimums.items():
            if actual < minimum:
                raise ValueError(
                    f"BMB830 module {self.module_id!r} requires at least {minimum} "
                    f"{name}; received {actual}."
                )


def _assessment_items(
    spec: StandardModuleSpec,
...
def build_module(spec: StandardModuleSpec) -> LocalizedLearningModule:
    """Build and validate one complete trilingual BMB830 learning module."""
...
def build_question_bank(
    ...
    """Build the stable 16-item objective bank for one module."""
