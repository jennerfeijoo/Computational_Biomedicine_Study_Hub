...
mean=9.77""",
    ...
    practices=(
        (
            ...
            (("Piensa en operaciones válidas.", "Think about valid operations.", "Tænk på gyldige operationer."),),
            (
        ...
        (
            ...
            (("anyDuplicated devuelve cero si no hay duplicados.", "anyDuplicated returns zero when none exist.", "anyDuplicated returnerer nul, når ingen findes."),),
        ...
        (
            ...
            (("Valida antes de modelar.", "Validate before modelling.", "Validér før modellering."),),
            (
        ...
        (
            ...
            (("R ordena niveles por defecto.", "R sorts levels by default.", "R sorterer niveauer som standard."),),
            (
    ...
    mcqs=(
        ("001", ("¿Qué objeto admite columnas de tipos distintos?", "Which object supports columns of different types?", "Hvilket objekt understøtter kolonner af forskellige typer?"), (("vector", ("Vector",) * 3), ("matrix", ("Matriz", "Matrix", "Matrix")), ("data_frame", ("Data frame",) * 3), ("factor", ("Factor", "Factor", "Faktor"))), "data_frame", ("Un data frame conserva tipos por columna.", "A data frame preserves column types.", "Et data frame bevarer kolonnetyper.")),
        ("002", ("¿Qué detecta una clave duplicada?", "What detects a duplicated key?", "Hvad opdager en dubleret nøgle?"), (("a", ("mean(id)",) * 3), ("b", ("anyDuplicated(id)",) * 3), ("c", ("levels(id)",) * 3), ("d", ("is.numeric(id)",) * 3)), "b", ("anyDuplicated devuelve cero si no hay duplicados.", "anyDuplicated returns zero when none exist.", "anyDuplicated returnerer nul, når ingen findes.")),
        ("003", ("¿Qué representa NA?", "What does NA represent?", "Hvad repræsenterer NA?"), (("zero", ("Cero", "Zero", "Nul")), ("missing", ("Valor ausente", "Missing value", "Manglende værdi")), ("empty", ("Cadena vacía", "Empty string", "Tom streng")), ("false", ("Falso", "False", "Falsk"))), "missing", ("NA no equivale a cero.", "NA is not zero.", "NA er ikke nul.")),
        ("004", ("¿Qué reduce estado oculto?", "What reduces hidden state?", "Hvad reducerer skjult tilstand?"), (("manual", ("Comandos manuales", "Manual commands", "Manuelle kommandoer")), ("script", ("Codificar transformaciones", "Encode transformations", "Kod transformationer")), ("workspace", ("Guardar workspace", "Save workspace", "Gem workspace")), ("attach", ("Usar attach", "Use attach", "Brug attach"))), "script", ("Un script reconstruye el estado.", "A script reconstructs state.", "Et script genskaber tilstanden.")),
        ("005", ("¿Qué objeto representa categorías?", "Which object represents categories?", "Hvilket objekt repræsenterer kategorier?"), (("factor", ("Factor", "Factor", "Faktor")), ("matrix", ("Matriz", "Matrix", "Matrix")), ("date", ("Fecha", "Date", "Dato")), ("numeric", ("Numérico", "Numeric", "Numerisk"))), "factor", ("Los factores conservan niveles.", "Factors preserve levels.", "Faktorer bevarer niveauer.")),
        ("006", ("¿Qué debe ocurrir antes de modelar?", "What should occur before modelling?", "Hvad bør ske før modellering?"), (("validate", ("Validar datos", "Validate data", "Validér data")), ("publish", ("Publicar", "Publish", "Publicér")), ("hide", ("Ocultar advertencias", "Hide warnings", "Skjul advarsler")), ("round", ("Redondear todo", "Round everything", "Afrund alt"))), "validate", ("La validación detecta errores tempranos.", "Validation detects early errors.", "Validering opdager tidlige fejl.")),
        ("007", ("¿Qué hace reproducible una simulación?", "What makes a simulation reproducible?", "Hvad gør en simulation reproducerbar?"), (("seed", ("Fijar set.seed", "Set set.seed", "Sæt set.seed")), ("sort", ("Ordenar", "Sort", "Sortér")), ("print", ("Imprimir", "Print", "Udskriv")), ("attach", ("Usar attach", "Use attach", "Brug attach"))), "seed", ("La semilla fija la secuencia pseudoaleatoria.", "The seed fixes the pseudo-random sequence.", "Seedet fastlægger den pseudo-tilfældige sekvens.")),
        ("008", ("¿Qué debe acompañar na.rm=TRUE?", "What should accompany na.rm=TRUE?", "Hvad bør ledsage na.rm=TRUE?"), (("missing", ("Número de NA", "Number of NA", "Antal NA")), ("colour", ("Color", "Colour", "Farve")), ("workspace", ("Workspace",) * 3), ("os", ("Sistema operativo", "Operating system", "Operativsystem"))), "missing", ("La exclusión debe cuantificarse.", "Exclusion must be quantified.", "Udelukkelse skal kvantificeres.")),
    ),
    true_false=(
        ("009", ("Una matriz mezcla texto y números sin coerción.", "A matrix mixes text and numbers without coercion.", "En matrix blander tekst og tal uden coercion."), False, ("Las matrices son homogéneas.", "Matrices are homogeneous.", "Matricer er homogene.")),
        ("010", ("NA y cero tienen el mismo significado.", "NA and zero have the same meaning.", "NA og nul har samme betydning."), False, ("NA indica ausencia.", "NA indicates absence.", "NA angiver fravær.")),
        ("011", ("Un factor puede conservar niveles no observados.", "A factor may retain unobserved levels.", "En faktor kan bevare ikke-observerede niveauer."), True, ("Los niveles forman parte de la definición.", "Levels are part of the definition.", "Niveauer er en del af definitionen.")),
        ("012", ("stopifnot hace verificable un supuesto.", "stopifnot makes an assumption verifiable.", "stopifnot gør en antagelse verificerbar."), True, ("La ejecución se detiene si falla.", "Execution stops if it fails.", "Kørslen stopper, hvis den fejler.")),
        ("013", ("Indexar por nombre suele resistir cambios de columnas.", "Name-based indexing usually resists column changes.", "Navnebaseret indeksering modstår normalt kolonneændringer."), True, ("Depende menos del orden físico.", "It depends less on physical order.", "Det afhænger mindre af fysisk rækkefølge.")),
        ("014", ("Guardar el workspace sustituye documentar el flujo.", "Saving the workspace replaces workflow documentation.", "At gemme workspace erstatter workflow-dokumentation."), False, ("El workspace conserva estado opaco.", "A workspace preserves opaque state.", "Et workspace bevarer uigennemsigtig tilstand.")),
        ("015", ("Una advertencia importa aunque exista salida.", "A warning matters even when output exists.", "En advarsel betyder noget, selv når der findes output."), True, ("Salida no garantiza validez.", "Output does not guarantee validity.", "Output garanterer ikke gyldighed.")),
        ("016", ("La aleatoriedad reproducible requiere una semilla declarada.", "Reproducible randomness requires a declared seed.", "Reproducerbar tilfældighed kræver et erklæret seed."), True, ("Sin semilla cambia la secuencia.", "Without a seed the sequence changes.", "Uden seed ændres sekvensen.")),
    ),
    tutor=(
        (
        ...
        (("Los vectores son homogéneos.", "Vectors are homogeneous.", "Vektorer er homogene."), ("Los factores representan categorías.", "Factors represent categories.", "Faktorer repræsenterer kategorier."), ("NA debe cuantificarse.", "NA must be quantified.", "NA skal kvantificeres."), ("El estado oculto rompe reproducibilidad.", "Hidden state breaks reproducibility.", "Skjult tilstand bryder reproducerbarhed.")),
        (("Tratar IDs como medidas.", "Treating IDs as measurements.", "At behandle ID'er som målinger."), ("Eliminar NA sin justificar.", "Removing NA without justification.", "At fjerne NA uden begrundelse."), ("Depender del workspace.", "Depending on a workspace.", "At afhænge af et workspace.")),
        (("¿Cuál es la unidad experimental?", "What is the experimental unit?", "Hvad er den eksperimentelle enhed?"), ("¿Qué tipo tiene cada variable?", "What type does each variable have?", "Hvilken type har hver variabel?"), ("¿Qué aserción detecta un error temprano?", "Which assertion detects an early error?", "Hvilken assertion opdager en tidlig fejl?")),
        (("Elige estructuras coherentes.", "Chooses coherent structures.", "Vælger sammenhængende strukturer."), ("Valida claves y ausencia.", "Validates keys and missingness.", "Validerer nøgler og manglende data."), ("Distingue ejecución de reproducibilidad.", "Distinguishes execution from reproducibility.", "Skelner mellem kørsel og reproducerbarhed.")),
        (("No inventar datos ni salida de R.", "Do not invent data or R output.", "Opfind ikke data eller R-output."), ("No recomendar eliminar NA sin justificar.", "Do not recommend removing NA without justification.", "Anbefal ikke fjernelse af NA uden begrundelse."), ("Responder en el idioma activo.", "Respond in the active language.", "Svar på det aktive sprog.")),
        ("SDU ODIN BMB830 active course description approved 2025-03-06", "R Core Team: R Language Definition", "R base documentation"),
    ),
)
