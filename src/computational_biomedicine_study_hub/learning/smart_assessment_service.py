...
class SmartAssessmentService(AIStudyService):
    ...
    def __init__(self, store, *, client: OllamaChatClient | None = None, model: str = "") -> None:
        base_client = client or OllamaChatClient()
        super().__init__(store, client=_SelectedModelChatClient(base_client, model))

    def generate_assessment(
        self, modules: tuple[LearningModule, ...], *, count: int = 10
        ...
        _interleave_by_module(result)
        final = tuple(result)
        self.store.save_generated_questions(final)
        return final

    def _generate_for_module(
        self, module: LearningModule, quota: int, weakness_weight: float
    ) -> tuple[GeneratedQuestion, ...]:
    ...
    def _validate_question(
        self, item: object, module: LearningModule, created_at: str
...
def programming_exercises(module: LearningModule):
    ...
    return tuple(
        exercise for exercise in module.practice_exercises if exercise.activity_type in code_types
    )


def _allocate_quotas(
    count: int, modules: tuple[LearningModule, ...], weights: dict[str, float]
) -> dict[str, int]:
...
def _interleave_by_module(questions: list[GeneratedQuestion]) -> None:
    """Mix source modules while keeping the sequence deterministic for tests."""
    buckets: dict[str, list[GeneratedQuestion]] = {}
    for question in questions:
        buckets.setdefault(question.module_id, []).append(question)
    mixed: list[GeneratedQuestion] = []
    while buckets:
        for module_id in list(buckets):
            bucket = buckets[module_id]
            if bucket:
                mixed.append(bucket.pop(0))
            if not bucket:
                del buckets[module_id]
    questions[:] = mixed
