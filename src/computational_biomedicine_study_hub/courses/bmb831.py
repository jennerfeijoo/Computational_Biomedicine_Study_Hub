"""BMB831 course registration and lazy authored-module navigation."""
...
from ..content.bmb831 import BUNDLES, LOCALIZED_BUNDLES
...
class BMB831ModuleReaderPage(ModuleReaderPage):
    """BMB831 reader that attaches editable R labs to safe authored examples."""
...
class BMB831Page(QWidget):
    """Construct each completed localized BMB831 module reader on first selection."""

    def __init__(self, locale: AppLocale = DEFAULT_LOCALE) -> None:
        super().__init__()
        self.setObjectName("bmb831CoursePage")
        ...
        self._reader_cache: dict[int, BMB831ModuleReaderPage] = {}
    ...
    def module_count(self) -> int:
        """Return the number of completed BMB831 modules."""
    ...
    def _reader_for_index(self, index: int) -> BMB831ModuleReaderPage:
        if not 0 <= index < self.module_count:
...
COURSE = CourseRegistration(
    ...
    ects=5,
    semester=1,
    summary="Estadística avanzada, datos clínicos sintéticos y pipelines ómicos.",
    ...
    localized_summaries={
        AppLocale.SPANISH_SPAIN: (
            "R avanzado, datos clínicos sintéticos, interpretación y pipelines ómicos."
        ),
        AppLocale.ENGLISH: (
            "Advanced R, synthetic clinical data, interpretation, and omics pipelines."
        ),
        AppLocale.DANISH_DENMARK: (
            "Avanceret R, syntetiske kliniske data, fortolkning og omik-pipelines."
        ),
    },
)
