"""Editable PySide6 surface for conservative local R execution."""
...
from ...learning.python_execution import ExecutionStatus
from ...learning.r_execution import (
    RCodeRunner,
    RExecutionRequest,
    RExecutionResult,
    RSubprocessRunner,
...
_R_LAB_STYLESHEET = PYTHON_LAB_STYLESHEET.replace("pythonLab", "rLab")
_STATUS_COPY = {
...
class RLabWidget(QFrame):
    """Let learners edit and execute one authored R example locally."""
    ...
    def __init__(
        ...
        *,
        ...
        runner: RCodeRunner | None = None,
        timeout_seconds: float = 3.0,
        parent: QWidget | None = None,
    ) -> None:
        super().__init__(parent)
        if not source.strip():
            raise ValueError("R labs require non-empty source code.")
        if not 0.1 <= timeout_seconds <= 15.0:
            raise ValueError("timeout_seconds must be between 0.1 and 15.0.")

        self.setObjectName("rLabWidget")
        self.setStyleSheet(_R_LAB_STYLESHEET)
        self._locale = locale
        self._original_source = source
        self._expected_output = expected_output
        self._runner = runner or RSubprocessRunner()
        self._timeout_seconds = timeout_seconds
        self._last_result: RExecutionResult | None = None

        layout = QVBoxLayout(self)
        layout.setContentsMargins(18, 16, 18, 16)
        layout.setSpacing(10)

        title = QLabel(f"R · {lab_text(locale, LabCopyKey.TITLE)}")
        title.setObjectName("rLabTitle")
        layout.addWidget(title)

        intro = QLabel(lab_text(locale, LabCopyKey.INTRO))
        intro.setObjectName("rLabIntro")
        intro.setWordWrap(True)
        layout.addWidget(intro)

        self._editor = self._code_editor(source, "rLabEditor", read_only=False)
        ...
        self._reset_button.setObjectName("rLabResetButton")
    ...
    @property
    def last_result(self) -> RExecutionResult | None:
    ...
    def run_code(self) -> None:
        """Execute the current R source with a hard timeout."""
    ...
    def _render_result(self, result: RExecutionResult) -> None:
    ...
    def _output_heading(text: str) -> QLabel:
        heading = QLabel(text)
        heading.setObjectName("rLabOutputHeading")
        return heading
    ...
    def _code_editor(
        ...
        *,
        ...
        RLabWidget._fit_output(editor, text)
        return editor
