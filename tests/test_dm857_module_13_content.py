"""Academic-content tests for DM857 module 13."""
...
TITLE_PREFIXES = {
    ...
    AppLocale.DANISH_DENMARK: "Biblioteker",
}


def test_module_13_has_complete_learning_components() -> None:
    module = MODULE_13_SCIENTIFIC_LIBRARIES
...
def test_module_13_materializes_completely_in_every_locale(locale: AppLocale) -> None:
    module = LOCALIZED_MODULE_13_SCIENTIFIC_LIBRARIES.materialize(locale)
    bank = materialize_module_13_question_bank(locale)
...
def test_module_13_objective_bank_has_stable_locale_independent_identity() -> None:
    ...
    reference = materialize_module_13_question_bank(AppLocale.SPANISH_SPAIN)
...
@pytest.mark.parametrize("locale", tuple(AppLocale))
def test_module_13_has_retrieval_ready_tutor_support(locale: AppLocale) -> None:
    module = LOCALIZED_MODULE_13_SCIENTIFIC_LIBRARIES.materialize(locale)
...
def test_module_13_examples_and_starter_code_compile() -> None:
    module = MODULE_13_SCIENTIFIC_LIBRARIES
...
def test_module_13_covers_varied_assessment_types() -> None:
    module = MODULE_13_SCIENTIFIC_LIBRARIES
...
def test_module_13_uses_safe_teaching_context_and_spain_terminology(
    locale: AppLocale,
