...
def test_modules_have_complete_learning_components(
    ...
    assert len(runtime_module.objectives) == (9 if module_id in {"dm857.m07", "dm857.m08"} else 8)
    assert len(runtime_module.concepts) == (9 if module_id in {"dm857.m07", "dm857.m08"} else 8)
...
def test_modules_materialize_completely_in_all_locales(
    ...
    for locale in AppLocale:
        ...
        assert len(module.objectives) == (9 if module_id in {"dm857.m07", "dm857.m08"} else 8)
        assert len(module.concepts) == (9 if module_id in {"dm857.m07", "dm857.m08"} else 8)
        assert len(module.worked_examples) == (6 if module_id in {"dm857.m07", "dm857.m08"} else 5)
