"""Academic-content tests for DM847 module 1."""
...
TITLE_PREFIXES = {
    AppLocale.SPANISH_SPAIN: "Información molecular",
    AppLocale.ENGLISH: "Molecular information",
    AppLocale.DANISH_DENMARK: "Molekylær information",
}


def test_module_01_is_registered_as_the_first_dm847_bundle() -> None:
    assert len(BUNDLES) == 1
    assert len(MODULES) == 1
    assert BUNDLES[0].content_version == "1.0.0"
    assert MODULES[0] is MODULE_01_MOLECULAR_INFORMATION
    assert MODULES[0].module_id == "dm847.m01"
    assert OBJECTIVE_QUESTION_BANKS["dm847.m01"] == OBJECTIVE_QUESTION_BANK_01


def test_module_01_has_complete_learning_components() -> None:
    module = MODULE_01_MOLECULAR_INFORMATION

    assert module.course_code == "DM847"
    assert module.module_id == "dm847.m01"
    ...
    assert len(OBJECTIVE_QUESTION_BANK_01) == 20
    assert all(module.tutor_support.source_basis)


@pytest.mark.parametrize("locale", tuple(AppLocale))
def test_module_01_materializes_completely_in_every_locale(locale: AppLocale) -> None:
    module = LOCALIZED_MODULE_01_MOLECULAR_INFORMATION.materialize(locale)
    bank = materialize_module_01_question_bank(locale)
...
def test_module_01_objective_bank_has_stable_locale_independent_identity() -> None:
    ...
    reference = materialize_module_01_question_bank(AppLocale.SPANISH_SPAIN)
...
@pytest.mark.parametrize("locale", tuple(AppLocale))
def test_module_01_has_retrieval_ready_tutor_support(locale: AppLocale) -> None:
    module = LOCALIZED_MODULE_01_MOLECULAR_INFORMATION.materialize(locale)
...
def test_module_01_examples_and_starter_code_compile() -> None:
    module = MODULE_01_MOLECULAR_INFORMATION
...
def test_module_01_covers_varied_assessment_types() -> None:
    module = MODULE_01_MOLECULAR_INFORMATION
    ...
    assert {ActivityType.MULTIPLE_SELECT, ActivityType.ORDERING}.issubset(assessment_types)
...
def test_module_01_uses_safe_teaching_context(
    locale: AppLocale,
