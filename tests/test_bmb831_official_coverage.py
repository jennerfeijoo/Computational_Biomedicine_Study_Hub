...
def test_bmb831_initial_audit_remains_conservative() -> None:
    ...
    assert "cannot satisfy omics-pipeline" in BMB831_SYNTHEA_BOUNDARY
    ...
    assert omics_rows
    assert all(requirement.status is CoverageStatus.GAP for requirement in omics_rows)
