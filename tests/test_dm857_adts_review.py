...
def _module(locale: AppLocale | str) -> LearningModule:
    localized = next(
        module for module in dm857.LOCALIZED_MODULES if module.module_id == "dm857.m11"
    )
...
def test_adt_extension_is_complete_and_locale_independent() -> None:
...
def test_adt_audit_records_review_and_named_sources() -> None:
    audit = {item.module_id: item for item in dm857.DM857_MODULE_SOURCE_AUDIT}
    item = audit["dm857.m11"]
...
def test_adt_concept_preserves_abstraction_boundaries() -> None:
    module = _module(AppLocale.ENGLISH)
    concept = next(
        item
        for item in module.concepts
        if item.concept_id == "representation-independence-and-contract-suites"
    )
    text = " ".join((concept.body, *concept.key_points)).casefold()

    assert "abstraction barrier" in text
    assert "observable" in text
    assert "invariant" in text
    assert "private" in text
    assert "contract suite" in text or "shared suite" in text


def test_adt_shared_contract_example_executes_deterministically(capsys) -> None:
    module = _module(AppLocale.ENGLISH)
    worked_example = next(
        item for item in module.worked_examples if item.example_id == "m11.bg.e01"
    )
