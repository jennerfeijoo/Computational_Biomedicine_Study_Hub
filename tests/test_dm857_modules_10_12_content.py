...
_EXTENDED_MODULES = {"dm857.m10", "dm857.m11", "dm857.m12"}
...
MODULE_CASES: tuple[ModuleCase, ...] = (
    (
        ...
        materialize_module_10_question_bank,
        {
            AppLocale.SPANISH_SPAIN: "Estructuras",
            AppLocale.ENGLISH: "Hierarchical",
            AppLocale.DANISH_DENMARK: "Hierarkiske",
        },
    ),
    (
        "dm857.m11",
        ...
        materialize_module_11_question_bank,
        {
            AppLocale.SPANISH_SPAIN: "Tipos abstractos",
            AppLocale.ENGLISH: "Abstract data",
            AppLocale.DANISH_DENMARK: "Abstrakte datatyper",
        },
    ),
    (
        "dm857.m12",
        ...
        materialize_module_12_question_bank,
        {
            AppLocale.SPANISH_SPAIN: "Clases",
            AppLocale.ENGLISH: "Classes",
            AppLocale.DANISH_DENMARK: "Klasser",
        },
    ),
)
...
def test_modules_have_complete_learning_components(
    ...
    assert len(runtime_module.objectives) == (9 if module_id in _EXTENDED_MODULES else 8)
    assert len(runtime_module.concepts) == (9 if module_id in _EXTENDED_MODULES else 8)
    assert len(runtime_module.worked_examples) == (6 if module_id in _EXTENDED_MODULES else 5)
    assert len(runtime_module.practice_exercises) == (13 if module_id in _EXTENDED_MODULES else 12)
    assert len(runtime_module.assessment_items) == (15 if module_id in _EXTENDED_MODULES else 14)
...
def test_modules_materialize_completely_in_all_locales(
    ...
    for locale in AppLocale:
        ...
        assert len(module.objectives) == (9 if module_id in _EXTENDED_MODULES else 8)
        assert len(module.concepts) == (9 if module_id in _EXTENDED_MODULES else 8)
        assert len(module.worked_examples) == (6 if module_id in _EXTENDED_MODULES else 5)
        assert len(module.practice_exercises) == (13 if module_id in _EXTENDED_MODULES else 12)
        assert len(module.assessment_items) == (15 if module_id in _EXTENDED_MODULES else 14)
        assert len(bank) == 30
...
def test_modules_have_retrieval_ready_tutor_support_and_valid_code(
    module_id: str,
    localized_module: LocalizedLearningModule,
    runtime_module: LearningModule,
    localized_bank: tuple[LocalizedAssessmentItem, ...],
    ...
    for exercise in runtime_module.practice_exercises:
...
def test_modules_cover_varied_assessment_and_safe_teaching_contexts(
    ...
    for locale in AppLocale:
        ...
        assert "protocol" in exported or "protokol" in exported
